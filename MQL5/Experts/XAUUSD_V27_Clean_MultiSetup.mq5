...
#property description "V28 Contextual Router: strict daily-flat intraday research and normalized risk"
...
input double MinBodyRatio=0.38;
...
input double TrailDistanceATR=0.55;
input int MaxHoldBars=20;
input double TimeExitMinProgressATR=0.35;
...
bool RouteQuality(string setup,int dir,double rs,double adx,double vr,double br,int b1,int b4){
 ...
 if(setup=="BREAKOUT"&&dir>0)return rs>=53&&rs<=66&&vr>=1.08&&br>=0.40;
 if(setup=="CONTINUATION"&&dir<0)return rs>=35&&rs<=47&&vr>=1.08&&br>=0.40;
 return true;
}
