...
#property description "V28 Contextual Router: natural setup x direction x session entries with normalized risk"
...
void Consider(int dir,string setup,double score,double atr,int hour,Candidate &c){if(score<MinSignalScore)return;string route;if(!Route(setup,dir,hour,route))return;if(c.dir==0||score>c.score){c.dir=dir;c.setup=setup;c.route=route;c.score=score;c.atr=atr;}}
...
bool Signals(Candidate &best){
 ...
 if(EnableBuy&&bb>0){double ph=HH(r,2,BreakoutLookback),pl=LL(r,2,SweepLookback);if(r[1].close>ph&&r[1].close>r[1].open&&br>=0.30&&vr>=0.90)Consider(1,"BREAKOUT",bb+24,atr,tm.hour,buy);bool touch=r[1].low<=s1+atr*PullbackTouchATR||r[2].low<=s1+atr*PullbackTouchATR;if(touch&&r[1].close>f1&&r[1].close>r[1].open&&rs>49&&rs<69)Consider(1,"PULLBACK",bb+22,atr,tm.hour,buy);if(r[1].close>r[2].high&&r[2].close>r[2].open&&f1>f2&&br>=0.35&&vr>=MinVolumeRatio)Consider(1,"CONTINUATION",bb+18,atr,tm.hour,buy);if(b1>0&&r[1].low<pl&&r[1].close>pl&&r[1].close>r[1].open&&rs>45)Consider(1,"SWEEP",bb+23,atr,tm.hour,buy);}
 if(EnableSell&&sb>0){double pl=LL(r,2,BreakoutLookback),ph=HH(r,2,SweepLookback);if(r[1].close<pl&&r[1].close<r[1].open&&br>=0.30&&vr>=0.90)Consider(-1,"BREAKOUT",sb+24,atr,tm.hour,sell);bool touch=r[1].high>=s1-atr*PullbackTouchATR||r[2].high>=s1-atr*PullbackTouchATR;if(touch&&r[1].close<f1&&r[1].close<r[1].open&&rs>31&&rs<51)Consider(-1,"PULLBACK",sb+22,atr,tm.hour,sell);if(r[1].close<r[2].low&&r[2].close<r[2].open&&f1<f2&&br>=0.35&&vr>=MinVolumeRatio)Consider(-1,"CONTINUATION",sb+18,atr,tm.hour,sell);if(b1<0&&r[1].high>ph&&r[1].close<ph&&r[1].close<r[1].open&&rs<55)Consider(-1,"SWEEP",sb+23,atr,tm.hour,sell);}
