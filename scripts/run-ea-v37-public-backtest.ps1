...
function Set-V41ProfileValue([string]$Text, [string]$Name, [string]$OldValue, [string]$NewValue) {
  $assignmentPattern = "(?m)^(\s*" + [regex]::Escape($Name) + "\s*=\s*)'" + [regex]::Escape($OldValue) + "'(\s*)$"
  $matches = [regex]::Matches($Text, $assignmentPattern)
  if ($matches.Count -ne 1) {
    throw "V41 wrapper transform expected exactly one locked-input assignment for $Name=$OldValue; found $($matches.Count)"
  }
  ...
  if ($Text -notmatch ("(?m)^\s*" + [regex]::Escape($Name) + "\s*=\s*'" + [regex]::Escape($NewValue) + "'\s*$")) {
    throw "V41 wrapper transform failed to verify canonical assignment: $Name=$NewValue"
  }
  return $Text
}
...
$replacements = [ordered]@{
  'V35_SELL_STRUCTURE.set' = 'V41_H8_OPPORTUNITY.set'
  'V35_SELL_STRUCTURE_journal.csv' = 'V41_H8_OPPORTUNITY_journal.csv'
  'V35 Sell Structure MT5 Backtest' = 'V41 Hour-8 Opportunity MT5 Backtest'
  'XAUUSD_V35_Sell_Structure_Quality_Gate' = 'XAUUSD_V41_H8_Opportunity_Expansion'
  'V35_CURRENT_RUN.txt' = 'V41_CURRENT_RUN.txt'
  'V35_PUBLIC_BACKTEST_FALLBACK_REPORT.html' = 'V41_PUBLIC_BACKTEST_FALLBACK_REPORT.html'
  'V35_MT5_ImportCustomSymbol.ini' = 'V41_MT5_ImportCustomSymbol.ini'
  'V35_MT5_Backtest.ini' = 'V41_MT5_Backtest.ini'
  'compile_v35_ea.log' = 'compile_v41_ea.log'
  'V35_WRAPPER_STAGE.txt' = 'V41_WRAPPER_STAGE.txt'
  'V35_LOCKED_RUNNER_PREVIEW.ps1' = 'V41_LOCKED_RUNNER_PREVIEW.ps1'
  'V35_LOCKED_RUNNER_CONSOLE.log' = 'V41_LOCKED_RUNNER_CONSOLE.log'
  'V35_LOCKED_RUNNER_EXIT_CODE.txt' = 'V41_LOCKED_RUNNER_EXIT_CODE.txt'
  'V35_LOCKED_RUNNER_ERROR.txt' = 'V41_LOCKED_RUNNER_ERROR.txt'
  'V35 locked runner' = 'V41 locked runner'
  'V35 runner lock failed' = 'V41 runner lock failed'
  'V35 EA' = 'V41 EA'
  'V35 Strategy Tester' = 'V41 Strategy Tester'
  'V35_${symbol}_${period}_${from}_${to}_model${model}' = 'V41_${symbol}_${period}_${from}_${to}_model${model}'
}
foreach ($entry in $replacements.GetEnumerator()) {
  if (!$text.Contains([string]$entry.Key)) { throw "V41 wrapper transform missing marker: $($entry.Key)" }
...
foreach ($entry in $profile.GetEnumerator()) {
  $text = Set-V41ProfileValue $text ([string]$entry.Key) ([string]$entry.Value[0]) ([string]$entry.Value[1])
}

$anchor = '  Write-Stage "literal-replacements-complete"'
if (!$text.Contains($anchor)) { throw "V41 wrapper transform missing V35 downloader injection anchor" }
$injection = @'
