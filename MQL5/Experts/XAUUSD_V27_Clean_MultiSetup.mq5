...
#property description "V28 Contextual Router: V31 strict continuation-buy 09-10 validation"
...
input string CSVJournalName="V28_CONTEXTUAL_journal.csv";
...
double HH(MqlRates &r[],int sh,int n){double v=r[sh].high;for(int i=sh+1;i<n+sh;i++)v=MathMax(v,r[i].high);return v;}
double LL(MqlRates &r[],int sh,int n){double v=r[sh].low;for(int i=sh+1;i<n+sh;i++)v=MathMin(v,r[i].low);return v;}
...
int MinuteOfDay(int hour,int minute){return hour*60+minute;}
...
bool HardDailyFlat(){MqlDateTime x;TimeToStruct(TimeCurrent(),x);if(x.day_of_week==0||x.day_of_week==6)return true;return MinuteOfDay(x.hour,x.min)>=MinuteOfDay(HardFlatHour,HardFlatMinute);}
bool LastEntryPassed(){MqlDateTime x;TimeToStruct(TimeCurrent(),x);return MinuteOfDay(x.hour,x.min)>=MinuteOfDay(LastEntryHour,LastEntryMinute);}
...
bool Route(string setup,int dir,int hour,string &name){name="";if(setup=="PULLBACK"&&dir>0&&hour==15){name="CORE_PULLBACK_BUY_15";return true;}if(setup=="CONTINUATION"&&dir<0&&hour>=7&&hour<9){name="CORE_CONTINUATION_SELL_07_08";return true;}if(setup=="SWEEP"&&dir<0&&hour>=13&&hour<15){name="CORE_SWEEP_SELL_13_14";return true;}if(setup=="CONTINUATION"&&dir>0&&hour>=9&&hour<11){name="EDGE_CONTINUATION_BUY_09_10_STRICT";return true;}return false;}
...
bool RouteQuality(string setup,int dir,double rs,double adx,double vr,double br,int b1,int b4){
 ...
 if(setup=="PULLBACK"&&dir>0)return rs>51&&rs<66&&vr>=1.00&&br>=0.36;
 if(setup=="SWEEP"&&dir<0)return rs>34&&rs<50&&vr>=1.00&&br>=0.36;
 if(setup=="BREAKOUT"&&dir>0)return rs>=53&&rs<=66&&vr>=1.04&&br>=0.38;
 if(setup=="CONTINUATION"&&dir<0)return rs>=35&&rs<=47&&vr>=1.04&&br>=0.38;
 if(setup=="CONTINUATION"&&dir>0)return rs>=53&&rs<=64&&vr>=1.08&&br>=0.42;
 return true;
}

bool RouteExtensionQuality(string route,double rs,double adx,double vr,double br,int b1,int b4){
 if(route!="EDGE_CONTINUATION_BUY_09_10_STRICT")return true;
 return b1>0&&b4>0&&rs>=53&&rs<=64&&adx>=26&&vr>=1.10&&br>=0.50;
}
...
void Consider(int dir,string setup,double score,double atr,int hour,double rs,double adx,double vr,double br,int b1,int b4,Candidate &c){if(score<MinSignalScore)return;if(!RouteQuality(setup,dir,rs,adx,vr,br,b1,b4))return;string route;if(!Route(setup,dir,hour,route))return;if(!RouteExtensionQuality(route,rs,adx,vr,br,b1,b4))return;if(c.dir==0||score>c.score){c.dir=dir;c.setup=setup;c.route=route;c.score=score;c.atr=atr;}}
...
bool Signals(Candidate &best){
 ...
 if(EnableBuy&&bb>0){double ph=HH(r,2,BreakoutLookback),pl=LL(r,2,SweepLookback);if(r[1].close>ph&&r[1].close>r[1].open&&br>=MinBodyRatio&&vr>=MinVolumeRatio)Consider(1,"BREAKOUT",bb+24,atr,tm.hour,rs,adx,vr,br,b1,b4,buy);bool touch=r[1].low<=s1+atr*PullbackTouchATR||r[2].low<=s1+atr*PullbackTouchATR;if(touch&&r[1].close>f1&&r[1].close>r[1].open&&rs>51&&rs<66)Consider(1,"PULLBACK",bb+22,atr,tm.hour,rs,adx,vr,br,b1,b4,buy);if(r[1].close>r[2].high&&r[2].close>r[2].open&&f1>f2&&br>=0.38&&vr>=MinVolumeRatio)Consider(1,"CONTINUATION",bb+18,atr,tm.hour,rs,adx,vr,br,b1,b4,buy);if(b1>0&&b4>0&&r[1].low<pl&&r[1].close>pl&&r[1].close>r[1].open&&rs>47&&br>=MinBodyRatio&&vr>=MinVolumeRatio)Consider(1,"SWEEP",bb+23,atr,tm.hour,rs,adx,vr,br,b1,b4,buy);}
 if(EnableSell&&sb>0){double pl=LL(r,2,BreakoutLookback),ph=HH(r,2,SweepLookback);if(r[1].close<pl&&r[1].close<r[1].open&&br>=MinBodyRatio&&vr>=MinVolumeRatio)Consider(-1,"BREAKOUT",sb+24,atr,tm.hour,rs,adx,vr,br,b1,b4,sell);bool touch=r[1].high>=s1-atr*PullbackTouchATR||r[2].high>=s1-atr*PullbackTouchATR;if(touch&&r[1].close<f1&&r[1].close<r[1].open&&rs>34&&rs<50)Consider(-1,"PULLBACK",sb+22,atr,tm.hour,rs,adx,vr,br,b1,b4,sell);if(r[1].close<r[2].low&&r[2].close<r[2].open&&f1<f2&&br>=0.38&&vr>=MinVolumeRatio)Consider(-1,"CONTINUATION",sb+18,atr,tm.hour,rs,adx,vr,br,b1,b4,sell);if(b1<0&&b4<0&&r[1].high>ph&&r[1].close<ph&&r[1].close<r[1].open&&rs<53&&br>=MinBodyRatio&&vr>=MinVolumeRatio)Consider(-1,"SWEEP",sb+23,atr,tm.hour,rs,adx,vr,br,b1,b4,sell);}
...
void OnTick(){ResetDay();Manage();ulong tk;if(Position(tk)||HardDailyFlat()||LastEntryPassed()||!SessionOpen()||tradesToday>=MaxTradesPerDay||(lastEntryTime>0&&TimeCurrent()-lastEntryTime<CooldownMinutes*60)||!NewBar())return;Candidate c;if(Signals(c))Open(c);}
