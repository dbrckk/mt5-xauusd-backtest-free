...
#property description "V31 Pruned Core Revalidation: V30 EDGE modules removed after cross-period dilution"
...
input string CSVJournalName="V31_PRUNED_CORE_journal.csv";
...
bool Open(const Candidate &c){MqlTick t;if(!SymbolInfoTick(Sym,t))return false;double entry=Price(c.dir>0?t.ask:t.bid),sl,tp;if(!Stops(c.dir,entry,c.atr,c.setup,sl,tp))return false;double lot,plan,actual;string dir=c.dir>0?"BUY":"SELL";if(!Size(c.dir,entry,sl,lot,plan,actual)){J("RISK_SKIP",dir,c.setup,c.score,entry,c.atr,"route="+c.route);return false;}
 trade.SetExpertMagicNumber((int)MagicNumber);trade.SetDeviationInPoints(SlippagePoints);trade.SetTypeFillingBySymbol(Sym);string comment="V31 "+dir+" "+c.setup;bool ok=c.dir>0?trade.Buy(lot,Sym,0,sl,tp,comment):trade.Sell(lot,Sym,0,sl,tp,comment);
