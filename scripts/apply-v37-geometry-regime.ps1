...
# Identity.
$ea = $ea.Replace('#property version "2.94"', '#property version "3.06"')
$ea = $ea.Replace('V35 Sell Structure Quality Gate: weak pullback route pruned', 'V48 H8-H9-H12 Continuation Expansion: hours 10-11 rejected, hour-12 independent validation')
$ea = $ea.Replace('input string CSVJournalName="V35_SELL_STRUCTURE_journal.csv";', 'input string CSVJournalName="V48_H8_H9_H12_CONTINUATION_journal.csv";')

# Retain V45 hour-08/hour-09 evidence, permanently prune failed hours 10 and 11, and add only fresh hour 12.
$routeOld = @'
...
$routeNew = @'
   ...
   if(setup=="CONTINUATION" && dir<0 && hour==12)
   {
      name="TEST_CONTINUATION_SELL_12";
      return true;
   }
'@
if (!$ea.Contains($routeOld)) { throw "V48 cannot find V35 sell route block." }
...
'@
if (!$ea.Contains($routeQualityV35)) { throw "V48 cannot find V35 RouteQuality block." }
$ea = $ea.Replace($routeQualityV35, $routeQualityV48)
...
# Broad deterministic continuation trigger. Routing isolates hour 12 for fresh evidence.
$biasAnchor = @'
...
'@
if (!$ea.Contains($biasAnchor)) { throw "V48 cannot find HTF bias insertion point." }
...
foreach ($marker in $required) {
  if (!$ea.Contains($marker)) { throw "V48 marker missing: $marker" }
}
...
foreach ($marker in $forbidden) {
  if ($ea.Contains($marker)) { throw "V48 forbidden marker present: $marker" }
}

Set-Content -Path $eaPath -Value $ea -Encoding UTF8
Write-Host "V48 H8-H9-H12 continuation expansion transform applied to $eaPath"
